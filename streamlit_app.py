...
def fetch_data(ticker, interval="1d", days=400):
    try:
        end_dt   = datetime.now()
        max_days = min(days, 59) if interval == "1h"  else \
                   min(days, 59) if interval == "15m" else \
                   min(days, 729) if interval == "4h" else days
...
def generate_signals(df, strategy):
    ...
    if strategy == "Triple SMA Ribbon (20/50/200)":
        ...
        tf_v   = (mid - 3.0 * df["ATR"]).values
        tc_v   = (mid + 3.0 * df["ATR"]).values
        ...
        sma50  = compute_sma(df["Close"], min(50,  len(df)))
        sma200 = compute_sma(df["Close"], min(200, len(df)))
        body      = (df["Close"] - df["Open"]).abs()
        avg_body  = body.rolling(min(14,len(df))).mean()
        ...
        swing_hi    = df["High"].rolling(n).max()
        swing_lo    = df["Low"].rolling(n).min()
        ...
        prev_hi     = df["High"].rolling(n).max().shift(1)
        prev_lo     = df["Low"].rolling(n).min().shift(1)
        ...
        upper   = (hl2 + mult * atr_v).copy()
        lower   = (hl2 - mult * atr_v).copy()
        ...
        tk_up = (df["Tenkan"] > df["Kijun"]) & (df["Tenkan"].shift(1) <= df["Kijun"].shift(1))
        tk_dn = (df["Tenkan"] < df["Kijun"]) & (df["Tenkan"].shift(1) >= df["Kijun"].shift(1))
        buy  = above_cloud & tk_up
        sell = below_cloud & tk_dn
        df["Signal"] = np.where(buy, 1, np.where(sell, -1, 0))
        df["Signal"] = df["Signal"].replace(0, np.nan).ffill().fillna(-1)
...
with tab_bt:
    ...
    if results:
        ...
        mkt_filter = st.radio("Filter:", ["All","US","India","Crypto","Commodity"], horizontal=True, key="mkt_filter")
        filtered   = results if mkt_filter=="All" else [r for r in results if r["Market"]==mkt_filter]
        if filtered:
            st.dataframe(pd.DataFrame([{
                "Asset":       r["Label"], "Market":  r["Market"],
                "Price":       fmt(r["Price"]), "Signal": r["Signal"],
        ...
        with ch1:
            chart_search = st.text_input("Search any stock", placeholder="AAPL, RELIANCE, BTC, Gold...",
        ...
        if row:
            ...
            if in_wl:
                if st.button(f"Remove {lbl} from Watchlist", key="wl_rm"):
                    st.session_state.watchlist.remove(sel_ticker); st.rerun()
            else:
                if st.button(f"★ Add {lbl} to Watchlist", key="wl_add", type="primary"):
            ...
            if log_t:
                ...
                def cst(v):
                    ...
                    return ""
                st.dataframe(ldf.style.map(cr,subset=["Return %"]).map(cst,subset=["Status"]),
                             use_container_width=True, hide_index=True)
            else:
...
with tab_bot:
    ...
    st.markdown("Each strategy runs on its **optimal timeframe** (15m/1H/4H/1D). Votes tallied across all 8.")

    if not ALPACA_AVAILABLE:
        st.error("Add `alpaca-py` to requirements.txt")
        st.code("alpaca-py")
        st.stop()
    ...
    if not api_key or not api_secret:
        ...
        3. Or save permanently: Streamlit → Manage App → Secrets:
        ```
        ALPACA_KEY = "your_key"
        ...
        try:
            ...
            st.info("**Vote thresholds:** 🇺🇸 US=5/8 · 🇮🇳 India=4/8 · 🪙 Crypto=3/8 · 🛢️ Commodities=4/8")

            bot_tickers = st.multiselect("Assets to scan",
                DEFAULT_US+DEFAULT_CR+DEFAULT_CM,
                default=["AAPL","NVDA","MSFT","BTC-USD","GC=F"],
