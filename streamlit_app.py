        ...
        for tab, period_label in zip(period_tabs, available_periods):
            with tab:
                ...
                with col_b:
                    ...
                    if buy_stocks:
                        st.dataframe(pd.DataFrame([{"Ticker":r["Ticker"].replace(".NS","").replace("-USD",""),
                            "Market":r["Market"],"Price":fmt(r["Price"]),
                            "Strat %":fmt(r["Net %"],"%"),"B&H %":fmt(r["B&H %"],"%"),"Win Rate":fmt(r["Win Rate"],"%")}
                ...
                if st.session_state.watchlist:
                    ...
                    if wl_rows:
                        st.dataframe(pd.DataFrame([{"Ticker":r["Ticker"].replace(".NS","").replace("-USD",""),
                            "Market":r["Market"],"Price":fmt(r["Price"]),"Signal":r["Signal"],
                ...
                with ch1:
                    chart_search = st.text_input("Stock", placeholder="Type ticker or name e.g. USAR, Apple...",
...
with tab_bot:
    ...
    st.markdown("Live signal scanning + automatic paper trade execution via Alpaca.")

    if not ALPACA_AVAILABLE:
        st.error("Alpaca library not installed. Add `alpaca-py` to your requirements.txt")
    ...
    if not api_key or not api_secret:
        ...
        st.markdown("""
        **Get free Alpaca Paper Trading keys:**
        1. Go to [alpaca.markets](https://alpaca.markets) and sign up free
        2. Click **Paper Trading** in the left menu
        ...
        try:
            ...
            a1.metric("Portfolio",    f"${pv:,.0f}")
            a2.metric("Cash",         f"${csh:,.0f}")
            a3.metric("P&L Today",    f"${pnl:+,.0f}")
            a4.metric("Buying Power", f"${bp:,.0f}")
            ...
            with s1:
                bot_strategy = st.selectbox("Strategy", STRATEGIES, key="bot_strat")
            with s2:
                bot_min_votes = st.slider("Min votes (out of 7)", 3, 7, 5, key="bot_votes")
            with s3:
            ...
            if scan_btn and bot_tickers:
                ...
                for i, ticker in enumerate(bot_tickers):
                    ...
                    try:
                        ...
                        raw = clean_df(raw)
                        if "Close" not in raw.columns or len(raw) < 60: continue

                        enriched = generate_signals(raw, bot_strategy)
                        ...
                        if buy_v >= bot_min_votes and buy_v > sell_v:
                            status = "BUY"
                        ...
                        elif sell_v >= bot_min_votes and sell_v > buy_v:
                            status = "SELL"
                            if ticker in positions:
                                try:
                                    ...
                                    action = f"Sell failed: {e}"
                            else:
                                action = "No position to sell"
                        ...
                        rows.append({
                            ...
                            "Sell Votes": sell_v,
                            "Action":  action,
                            "In Portfolio": "Yes" if ticker in positions else "No"
                        })

                    except Exception as e:
                        rows.append({"Ticker":ticker,"Price":"—","Signal":"ERROR","Buy Votes":0,"Sell Votes":0,"Action":str(e),"In Portfolio":"—"})
                ...
                st.dataframe(pd.DataFrame(rows).style.map(sig_col2, subset=["Signal"]),
                             use_container_width=True, hide_index=True)
            ...
            try:
                ...
                if positions:
                    pos_df = pd.DataFrame([{
                        ...
                        "Curr Price":float(p.current_price),
                    ...
                    st.dataframe(pos_df.style.map(pl_col, subset=["P&L $","P&L %"]),
                                 use_container_width=True, hide_index=True)

                    close_all = st.button("Close All Positions", type="primary")
                    if close_all:
            ...
            st.error(f"Could not connect to Alpaca: {e}. Check your API keys.")